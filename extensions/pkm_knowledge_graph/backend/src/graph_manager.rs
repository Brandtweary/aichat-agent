...
impl GraphManager {
    ...
    fn should_save(&self) -> bool {
        ...
        if minutes_since_save >= SAVE_INTERVAL_MINUTES {
            debug!("Time-based save triggered: {} minutes since last save", minutes_since_save);
            return true;
        }
        
        // Check operation-based save (every 10 operations)
        if self.operations_since_save >= SAVE_OPERATION_THRESHOLD {
            debug!("Operation-based save triggered: {} operations since last save", self.operations_since_save);
            return true;
        }
